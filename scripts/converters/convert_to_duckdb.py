                ...
                for fips_data in time_data.values():
                    if isinstance(fips_data, list):
                        for transition in fips_data:
                            total_transitions += len([k for k in transition.keys() if k not in ['_row', 't1']])
                                ...
                                if from_landuse in landuse_lookup:
                                    ...
                                    for to_code, acres in transition.items():
                                        if to_code not in ['_row', 't1'] and to_code in landuse_lookup:
                                            ...
                                            if acres > 0:  # Only record actual transitions
...
def main():
    ...
    console.print(Panel.fit(
        ...
        "[yellow]Converting nested JSON to normalized relational database[/yellow]",
        border_style="blue"
    ))
    
    input_file = "data/raw/county_landuse_projections_RPA.json"
    output_file = "data/processed/landuse_analytics.duckdb"
    
    converter = LanduseDataConverter(input_file, output_file)
