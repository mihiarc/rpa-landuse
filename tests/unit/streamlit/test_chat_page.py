...
class TestChatPage:
    ...
    @patch('pages.chat.LanduseNaturalLanguageAgent')
    @patch('pages.chat.os.getenv')
    def test_initialize_agent(self, mock_getenv, mock_agent_class):
        """Test agent initialization"""
        mock_getenv.return_value = "/path/to/db.duckdb"
        ...
        mock_agent_class.assert_called_once_with("/path/to/db.duckdb")
    
    @patch('pages.chat.st')
    def test_display_message_user(self, mock_st):
        ...
        display_message("user", "Hello agent")
        
        mock_st.chat_message.assert_called_with("user")
        mock_chat_message.markdown.assert_called_with("Hello agent")
