...
class TestExplorerPage:
        ...
        return {
            "dim_scenario": {
                "columns": pd.DataFrame(
                    {"column_name": ["scenario_id", "scenario_name"], "column_type": ["INTEGER", "VARCHAR"]}
                ),
                "row_count": 20,
                "sample_data": pd.DataFrame({"scenario_id": [1, 2], "scenario_name": ["RCP45_SSP1", "RCP85_SSP5"]}),
            },
            "fact_landuse_transitions": {
                "columns": pd.DataFrame(
                    {"column_name": ["transition_id", "acres"], "column_type": ["BIGINT", "DECIMAL"]}
                ),
                "row_count": 5400000,
                "sample_data": pd.DataFrame({"transition_id": [1, 2], "acres": [1000.5, 2000.7]}),
            },
        }
    ...
    def sample_query_results(self):
        ...
        return pd.DataFrame(
            {"state_code": ["06", "48", "36"], "total_acres": [1000000, 800000, 600000], "county_count": [58, 254, 62]}
        )
    ...
    def test_allowed_tables_includes_required(self):
        ...
        required_tables = {"dim_scenario", "dim_geography", "dim_time", "dim_landuse", "fact_landuse_transitions"}
    ...
    def test_get_table_schema_returns_dict(self, mock_get_conn, mock_connection, sample_schema_info):
        ...
        mock_connection.list_tables.return_value = pd.DataFrame({"table_name": ["dim_scenario", "dim_geography"]})
